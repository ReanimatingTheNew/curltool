    ...
    # 设置表单数据
        ...
        for key, value in curl_dict["form_data"].items():
            if value["type"] == "file":
                ...
                code.append(f"    let file_name = std::path::Path::new(file_path).file_name().unwrap().to_str().unwrap();")
                code.append(f"    let mut file = File::open(file_path)?;")
                code.append(f"    let mut buffer = Vec::new();")
                code.append(f"    file.read_to_end(&mut buffer)?;")
                code.append(f"    form = form.part(\"{key}\", reqwest::blocking::multipart::Part::bytes(buffer).file_name(file_name.to_string()));")
            else:
                code.append(f"    // 添加表单字段 {key}")
                code.append(f"    form = form.text(\"{key}\", \"{value['value']}\");")
...
def to_swift(curl_dict):
    ...
    code = [
        ...
        "",
        "// 创建URL请求",
        f"guard let url = URL(string: \"{curl_dict['url']}\") else {{",
        "    print(\"Invalid URL\")",
        "    exit(1)",
        "}",
        "",
    ...
    # 设置表单数据
        ...
        for key, value in curl_dict["form_data"].items():
            ...
            if value["type"] == "file":
                ...
                code.append(f"// 注意：在实际应用中，您需要读取文件内容并添加")
