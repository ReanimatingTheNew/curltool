...
import re
import shlex
from curl_to_code import parse_curl_to_dict, to_python, to_javascript, to_nodejs, to_php, php_array_from_dict
from java_csharp_converter import to_java, to_csharp
from rust_swift_converter import to_rust, to_swift
from go_ruby_converter import to_go, to_ruby

# 支持的语言列表
SUPPORTED_LANGUAGES = [
    'python', 'javascript', 'nodejs', 'php', 
    'java', 'csharp', 'rust', 'swift',
    'go', 'ruby'
]

def convert_curl_to_code(curl_command, language):
    """
    将curl命令转换为指定编程语言的代码
    ...
    """
    # 解析curl命令
    curl_dict = parse_curl_to_dict(curl_command)
    ...
    # 根据目标语言调用相应的转换函数
    if language == 'python':
        return to_python(curl_dict)
    elif language == 'javascript':
        return to_javascript(curl_dict)
    elif language == 'nodejs':
        return to_nodejs(curl_dict)
    elif language == 'php':
        return to_php(curl_dict)
    elif language == 'java':
        return to_java(curl_dict)
    elif language == 'csharp':
        return to_csharp(curl_dict)
    elif language == 'rust':
        return to_rust(curl_dict)
    elif language == 'swift':
        return to_swift(curl_dict)
    else:
    ...
    if curl_dict["data"] is not None:
        ...
        if isinstance(curl_dict["data"], dict):
            if curl_dict["headers"].get("Content-Type", "").lower() == "application/json":
                ...
                code.append(f"request.body = '" + "&".join(form_parts) + "'")
        else:
            # 字符串形式的data
...
# 更新支持的语言列表
SUPPORTED_LANGUAGES.extend(['go', 'ruby'])
...
def convert_curl_to_code(curl_command, language):
    ...
    if language == 'python':
        ...
        return to_go(curl_dict)
    elif language == 'ruby':
        return to_ruby(curl_dict)
    else:
        return f"// 不支持的语言: {language}"
