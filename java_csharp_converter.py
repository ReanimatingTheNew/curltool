...
def to_java(curl_dict):
    ...
    code.append("            HttpURLConnection connection = (HttpURLConnection) url.openConnection();")
    ...
    if not curl_dict["allow_redirects"]:
        code.append("            // 禁用自动重定向")
        code.append("            connection.setInstanceFollowRedirects(false);")
    ...
    if curl_dict["auth"] is not None and curl_dict["auth"]["type"] == "basic" and "username" in curl_dict["auth"]:
        ...
        code.append("            connection.setRequestProperty(\"Authorization\", \"Basic \" + encodedAuth);")
    
    # 设置请求体
    if curl_dict["data"] is not None or curl_dict["form_data"] is not None:
        code.append("            // 设置输出")
        code.append("            connection.setDoOutput(true);")
        ...
        if curl_dict["data"] is not None:
            ...
            if isinstance(curl_dict["data"], dict):
                if curl_dict["headers"].get("Content-Type", "").lower() == "application/json":
                    ...
                    code.append("            try (OutputStream os = connection.getOutputStream()) {")
                    code.append("                byte[] input = formData.getBytes(StandardCharsets.UTF_8);")
                    code.append("                os.write(input, 0, input.length);")
                    code.append("            }")
            else:
                # 字符串形式的data
            ...
            code.append("            String boundary = \"----WebKitFormBoundary7MA4YWxkTrZu0gW\";")
            code.append("            connection.setRequestProperty(\"Content-Type\", \"multipart/form-data; boundary=\" + boundary);")
            code.append("            try (OutputStream os = connection.getOutputStream()) {")
            ...
            for key, value in curl_dict["form_data"].items():
                ...
                if value["type"] == "file":
                    ...
                    code.append("                writer.append(\"Content-Type: application/octet-stream\").append(\"\\r\\n\");")
                    code.append("                writer.append(\"\\r\\n\");")
                    code.append("                writer.flush();")
                    code.append("                // 注意：在实际应用中，您需要读取文件内容并写入")
                    code.append("                // 这里仅作为示例")
                    code.append(f"                try (FileInputStream fis = new FileInputStream(\"{value['path']}\")) {{")
                    code.append("                    byte[] buffer = new byte[1024];")
                    code.append("                    int length;")
                    code.append("                    while ((length = fis.read(buffer)) > 0) {")
                    code.append("                        os.write(buffer, 0, length);")
                    code.append("                    }")
                    code.append("                    os.flush();")
                    code.append("                }")
                    code.append("                writer.append(\"\\r\\n\");")
                else:
    ...
    # 获取响应
    code.append("            // 获取响应")
    code.append("            int responseCode = connection.getResponseCode();")
    code.append("            System.out.println(\"Response Code: \" + responseCode);")
    code.append("            ")
    code.append("            // 获取响应头")
    code.append("            Map<String, String> responseHeaders = new HashMap<>();")
    code.append("            for (int i = 0; ; i++) {")
    code.append("                String headerName = connection.getHeaderFieldKey(i);")
    code.append("                String headerValue = connection.getHeaderField(i);")
    code.append("                if (headerName == null && headerValue == null) {")
    code.append("                    break;")
    code.append("                }")
    code.append("                if (headerName != null) {")
    code.append("                    responseHeaders.put(headerName, headerValue);")
    code.append("                }")
    code.append("            }")
    code.append("            System.out.println(\"Response Headers: \" + responseHeaders);")
    code.append("            ")
    code.append("            // 读取响应内容")
    code.append("            StringBuilder response = new StringBuilder();")
    code.append("            try (Scanner scanner = new Scanner(")
    code.append("                    responseCode >= 400 ? connection.getErrorStream() : connection.getInputStream())) {")
    code.append("                while (scanner.hasNextLine()) {")
    code.append("                    response.append(scanner.nextLine());")
    code.append("                }")
    code.append("            }")
    code.append("            System.out.println(\"Response Body: \" + response.toString());")
    
    # 关闭连接
    code.append("            // 关闭连接")
    code.append("            connection.disconnect();")
...
def to_csharp(curl_dict):
    ...
    if curl_dict["auth"] is not None and curl_dict["auth"]["type"] == "basic" and "username" in curl_dict["auth"]:
        ...
        code.append("            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(\"Basic\", Convert.ToBase64String(authToken));")
    ...
    # 设置表单数据
        ...
        for key, value in curl_dict["form_data"].items():
            if value["type"] == "file":
                ...
                code.append("            fileContent.Headers.ContentType = MediaTypeHeaderValue.Parse(\"application/octet-stream\");")
